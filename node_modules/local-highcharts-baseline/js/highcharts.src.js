...
// encapsulated variables
    ...
    Axis.prototype = {
        ...
        defaultOptions: {
            ...
            baseLine: false,  // 是否应用基线
            baseLineName: '基线', // 基线legend名称
            baseLineData: [], // 基线数据
            baseLineColor: '#F05731', // 基线的颜色
            baseLineDashArray: '10,10', // 基线样式
            baseLineWidth: 2, // 基线线条宽度
            baseLineFormatter: function(tick) { // 基线提示格式化函数
                return '<span style="font-size: 10px">'+ tick.axis.options.baseLineName +'</span><br/><span style="color:'+tick.axis.options.baseLineColor+'">●</span>' + tick.value +'<br/>'
            }
        },
        ...
        init: function (chart, userOptions) {
            ...
            // 基线仅绑定到Y轴，如果Y轴不存在，则基线也不存在
            if(axis.coll === 'yAxis') {
                // 获取X轴
                var xAxis = chart.xAxis[0];
                // 是否显示基线
                axis.baseLine = chart.hasBaseLine();

                if(axis.baseLine === true) {
                    var boptions = chart.options.baseLine
                    // 为数组时，表示可以画两条基线
                    if(isArray(boptions)){
                        // 判断当前竖坐标是否靠右侧
                        if(axis.opposite === true){
                            // 取基线
                            boptions = boptions.filter(function(option){
                    ...
                    if(boptions){
                        if(boptions.name)
                        axis.options.baseLineName = boptions.name;
                        if(boptions.data)
                            axis.options.baseLineData = boptions.data;
                        if(boptions.width)
                            axis.options.baseLineWidth = boptions.width
                        if(boptions.color)
                            axis.options.baseLineColor = boptions.color
                        if(boptions.dashArray)
                            axis.options.baseLineDashArray = boptions.dashArray
                        if(boptions.formatter && typeof boptions.formatter === 'function') {
                            axis.options.baseLineFormatter = boptions.formatter
                        }
                    }

                    axis.baseLines = axis.options.baseLineData.map(function(d) {
                        return new BaseLine(axis, d.value, d.start, d.end, d.startText, d.endText);
                    })
                } else {
                    axis.baseLines = [];
                    axis.baseConnectionLines = [];
                }
            }
        },
        ...
        getSeriesExtremes: function () {
            ...
            if(axis.coll === 'yAxis') {
                if(axis.baseLine) {
                    axis.baseLines.forEach(function(line) {
                        // 基线最小值
                        if (line.value < axis.dataMin || axis.dataMin === null) {
                            axis.dataMin = line.value;
                        }
                        // 基线最大值
                        if(line.value > axis.dataMax || axis.dataMax === null) {
                            axis.dataMax = line.value;
                        }
                    });
                }
            }
        ...
        redraw: function () {
            ...
            if (this.visible) {
                ...
                // 重新绘制基线
                this.chart.renderBaseLines(true);
            }
    ...
    Pointer.prototype = {
        ...
        runPointActions: function (e) {
            ...
            chart.yAxis.forEach(function(yAxis){
                // 如果显示基线
            if(yAxis.baseLine && chart.isVisibleOfBaseline()) {
                // 获取鼠标位置
                var mousePosition = {
                    x: e.chartX ,
                    y: e.chartY
                ...
                // 判定是否显示tooltip
                yAxis.baseLines.forEach(function(tick) {
                    var tickWidth = tick.width;
                    if(tick.x1 <= mousePosition.x
                        && mousePosition.x <= tick.x2
                        && tick.top -2 <= mousePosition.y
                        && mousePosition.y <= tick.top + tickWidth + 2) {
                        var position = { plotX: mousePosition.x - chart.plotLeft, plotY: mousePosition.y - chart.plotTop };
                        if (tooltip.isHidden) {
                            stop(tooltip.label);
                            tooltip.label.attr({
                                opacity: 1,
                        ...
                        if(chart.hoverPoint){
                            chart.hoverPoint.setState(NORMAL_STATE)
                        }
                        if(chart.hoverPoints){
                            chart.hoverPoints.forEach(function(item){
                                item.setState(NORMAL_STATE)
                            })
                        }

                        var color = tick.line.attr('stroke');
                        var text = yAxis.options.baseLineFormatter(tick);
                        tooltip.label.attr({
                            'stroke': color,
                            'text': text
                        });
    ...
    Legend.prototype = {
        ...
        colorizeItem: function (item, visible) {
            var legend = this,
                ...
                key,
                val;
            var hasBaseLine = legend.chart.hasBaseLine() && (item.options.type === 'baseLine' || item.options.type === 'baseLineArea');
            if (legendItem) {
            ...
            if (legendLine) {
                ...
                if(hasBaseLine) {
                    ...
                    if(visible) {
                        legendLine.attr({
                            'stroke': legend.chart.yAxis.filter(function(yAxis){
                                if(item.options.yAxis === 0){
                                    return !yAxis.opposite
                                }else if(item.options.yAxis === 1){
                                    return yAxis.opposite
                                }
                                return true
                            })[0].options.baseLineColor
                        })
                    }
                }
            }

            if (legendSymbol && !hasBaseLine) {
    ...
    Chart.prototype = {
        ...
        renderBaseLines: function(isRedraw) {
            var chart = this;
            this.yAxis.forEach(function(yAxis){
                if(yAxis.baseLine && yAxis.baseLines) {
                    if(!yAxis.baseLineGroup){
                        yAxis.baseLineGroup = chart.renderer.g('baseLine')
                  .attr({ zIndex: yAxis.options.baseLineZIndex || 10 })
                  .add(chart.seriesGroup);
               }
            yAxis.baseLines.forEach(function(line) {
              line.render(false, 1);
            });
            chart.setBaseConnectionLines(yAxis, isRedraw);
            chart.renderBaseConnectionLines(yAxis)
          }
            })
       },

        setBaseConnectionLines: function(yAxis, isRedraw) {
            if(!isRedraw){
                yAxis.baseConnectionLines = [];
                yAxis.baseLines.reduce(function(left, right) {
                  if(left && right){
                    yAxis.baseConnectionLines.push(new BaseConnectionLine(yAxis, left.x2, left.top, right.top, yAxis.options.baseLineWidth, yAxis.options.baseLineColor, yAxis.options.baseLineDashArray));
                  }
                  return right
                }, null);
            } else {
                ...
                yAxis.baseLines.reduce(function(left, right) {
                    if(left && right){
                        var line= yAxis.baseConnectionLines[count];
                      line.x = left.x2;
                      line.y1 = left.top;
                      line.y2 = right.top;
                      count ++;
        ...
        renderBaseConnectionLines: function(yAxis) {
            yAxis.baseConnectionLines.forEach(function(line) {
                line.render()
            });
        },

        // 是否有基线显示，注意基线只能有一条
        isVisibleOfBaseline: function(){
            var series = this.series.filter(function(s){
                return (s.options.type === 'baseLine' || s.options.type === 'baseLineArea') && s.visible === true
            })
            if(series.length ===0 )
                return false
            return true
        ...
        */
        ...
        hasBaseLine: function () {
            return typeof this.options.baseLine !== 'undefined' && this.options.series.findIndex(function(s){
                return s.type === 'baseLine' || s.type === 'baseLineArea'
            }) > -1;
        }
    }; // end Chart
    ...
    */
    ...
    extend(Series.prototype, {
        ...
        setState: function (state) {
            ...
            // 基线的显示效果切换
            if(series.options.type === 'baseLine' || series.options.type === 'baseLineArea') {
          series.chart.yAxis.forEach(function(yAxis){
              if(yAxis.baseLine && (series.options.yAxis && yAxis.opposite|| !series.options.yAxis && !yAxis.opposite)) {
              yAxis.baseLines.forEach(function(tick) {
                tick.line.attr({
                    'stroke-width': state === HOVER_STATE ? lineWidth: tick.width
                })
              })
              yAxis.baseConnectionLines.forEach(function(tick) {
                tick.line.attr({
                    'stroke-width': state === HOVER_STATE ? lineWidth: tick.width
                })
              })
        ...
        setVisible: function (vis, redraw) {
            ...
            // 基线的显示效果切换
            if(this.options.type === 'baseLine' || this.options.type === 'baseLineArea') {
          chart.yAxis.forEach(function(yAxis){
              if(yAxis.baseLine && (series.options.yAxis && yAxis.opposite|| !series.options.yAxis && !yAxis.opposite)) {
              yAxis.baseLines.forEach(function(tick) {
                            if(tick && tick.line)
                  tick.line.attr({
                      visibility: showOrHide === 'show'? 'visible' : 'hidden'
                  })
              });
              yAxis.baseConnectionLines.forEach(function(tick) {
                            if(tick && tick.line)
    ...
    function BaseConnectionLine(axis, x, y1, y2, width, color, dashArray) {
        this.axis = axis;
        this.x = x;
        this.y1 = y1;
        this.y2 = y2;
    ...
    BaseConnectionLine.prototype = {
        render: function() {
            var chart = this.axis.chart,
                renderer = chart.renderer;
            var linePath = ['M', this.x , this.y1 , 'L' , this.x , this.y2];
            if(!this.line) {
                this.line = renderer.path(linePath).attr({
                    'stroke': this.color,
                    'stroke-width': this.width,
                    'stroke-dasharray': this.dashArray
                }).add(this.axis.baseLineGroup);
            } else {
                this.line.animate({
                    d: linePath
                })
